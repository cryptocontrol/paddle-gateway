...
const PADDLE_PUBLIC_KEY = ''// Even if u ignore this parameter it do work
const PADDLE_VENDOR_ID = ''
const PADDLE_API_KEY = ''
const PADDLE_PLAN_ID = ''
const PORT = 8080

// These const can be used to integrate with keygen not required to integrate with paddle
const KEYGEN_PRODUCT_TOKEN = ''
const KEYGEN_ACCOUNT_ID = ''
const KEYGEN_POLICY_ID = ''
...
const callaback = (userid, type, tx_details) => {
  // cryptocontrol code goes here
}


// Verify Paddle webhook data using our public key

const verifyPaddleWebhook = async data => {
...
app.post('/paddle-webhooks', async (req, res) => {
  ...
  if (!verifyPaddleWebhook(paddleEvent)) {
    return res.sendStatus(200) // Webhook was not sent from Paddle
  }
